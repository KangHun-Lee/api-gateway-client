...
async function saveRequest() {
  try {
    const result = await showConfirmDialog(
      t('api.notices.add.save.header'),
      t('api.notices.add.save.message')
    )
    ...
    errorToast(t('api.notices.add.save.error'))
  }
}

async function saveNotices() {
  try {
    await NoticesService.saveNotices(
    ...
    successToast(t('api.notices.add.save.success'))
    router.push('/notice')
  } catch (error) {
    errorToast(t('api.notices.add.save.error'))
  }
  ...
  <div>
    <div>
      <Card>
        <template #title>
          <div class="section">
            <strong>{{ t('view.notices.title.save') }}</strong>
          </div>
        </template>
        <template #content>
          ...
          <div class="button-section">
            ...
            <Button
              ...
              @click="showPreview()"
            />
            <Button :label="t('view.notices.label.detail.save')" @click="saveRequest()" />
          </div>
        </template>
      </Card>
    </div>
